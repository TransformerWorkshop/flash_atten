...
`FA_K_BUF_REAL` 保存当前 K tile，并向 QK 计算路径提供 512-bit row reads。结构与 `FA_Q_BUF_REAL` 相同，是 `FA_REG_TILE_BUF_REAL` 的 wrapper。
...
| 端口 | 粒度 | 用途 |
|---|---:|---|
| write | 128-bit beat，4 个 32-bit words | K tile load |
| read | 512-bit row slice | QK GEMM 输入 B |
