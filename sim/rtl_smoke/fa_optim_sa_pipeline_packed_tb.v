...
module fa_optim_sa_pipeline_packed_tb;
    reg         clk;
    reg         rstn;
    reg         clear;
    reg         start;
    ...
    FA_OPTIM_SA_PIPELINE_PACKED_PROTOTYPE dut (
        .clk(clk),
        .rstn(rstn),
        .clear(clear),
        .start(start),
    ...
    initial begin
        ...
        if (done !== 1'b1) begin
            $display("FAIL: timeout waiting for packed done, wait_count=%0d cycles=%0d", wait_count, cycles);
            $fatal(1);
        end
        ...
        if (busy !== 1'b0) begin
            $display("FAIL: packed busy should be low after done");
            error_count = error_count + 1;
        end
        if (^packed_buffer_probe_data === 1'bx) begin
            $display("FAIL: packed_buffer_probe_data contains X");
            error_count = error_count + 1;
        end

        if (error_count == 0) begin
            $display("PASS: fa_optim_sa_pipeline_packed_tb cycles=%0d sa_busy=%0d feeder_busy=%0d row_state_busy=%0d",
                     cycles, sa_busy_cycles, feeder_busy_cycles, row_state_busy_cycles);
            $finish;
        end

        $display("FAIL: fa_optim_sa_pipeline_packed_tb errors=%0d", error_count);
        $fatal(1);
    end
endmodule
