...
module fa_optim_sa_pipeline_tb;
    reg         clk;
    reg         rstn;
    reg         clear;
    reg         start;
    ...
    wire [63:0] buffer_probe_data;

    integer error_count;
    integer wait_count;

    FA_OPTIM_SA_PIPELINE_PROTOTYPE dut (
        .clk(clk),
        .rstn(rstn),
        .clear(clear),
        .start(start),
        ...
        .buffer_probe_data(buffer_probe_data)
    );
    ...
    initial begin
        ...
        if (done !== 1'b1) begin
            $display("FAIL: timeout waiting for done, wait_count=%0d cycles=%0d", wait_count, cycles);
            $fatal(1);
        end

        expect32(cycles, 32'd2242, "cycles");
        expect32(sa_busy_cycles, 32'd8704, "sa_busy_cycles");
        expect32(feeder_busy_cycles, 32'd2176, "feeder_busy_cycles");
        ...
        expect32(pv_feed_count, 32'd0, "pv_feed_count");
        expect32(cluster_wait_task_count, 32'd264, "cluster_wait_task_count");
        expect32(feeder_wait_slot_count, 32'd66, "feeder_wait_slot_count");
        expect32(pv_wait_row_update_count, 32'd0, "pv_wait_row_update_count");
        expect32(active0_count, 32'd17, "active0_count");
        expect32(active1_count, 32'd33, "active1_count");
        expect32(active2_count, 32'd32, "active2_count");
        expect32(active3_count, 32'd33, "active3_count");
        expect32(active4_count, 32'd2127, "active4_count");

        if (busy !== 1'b0) begin
            $display("FAIL: busy should be low after done");
            error_count = error_count + 1;
        end
        if (^buffer_probe_data === 1'bx) begin
            $display("FAIL: buffer_probe_data contains X");
            error_count = error_count + 1;
        end

        if (error_count == 0) begin
            $display("PASS: fa_optim_sa_pipeline_tb cycles=%0d sa_busy=%0d feeder_busy=%0d row_state_busy=%0d",
                     cycles, sa_busy_cycles, feeder_busy_cycles, row_state_busy_cycles);
            $finish;
        end

        $display("FAIL: fa_optim_sa_pipeline_tb errors=%0d", error_count);
        $fatal(1);
    end
endmodule
