...
module QUANT #(
	...
	generate
		...
		for (li = 0; li < GEMM_Y_DIM; li = li + 1) begin : gen_quant_lane
			...
			wire signed [31:0] inv_scale_q16_16 = quant_inv_scale[scale_idx*32 +: 32];
			wire signed [IN_W-1:0] lane_in = in_data[li*IN_W +: IN_W];
			...
			assign out_data[li*DATA_WIDTH +: DATA_WIDTH] = lane_out;
		end
	endgenerate
