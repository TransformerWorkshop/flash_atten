...
module FA_OACC_UPDATE_Q4_BLOCK_REAL (
    input  wire           clk,
    input  wire           rstn,
    input  wire           clear,
    input  wire           req_valid,
    output wire           req_ready,
    input  wire [127:0]   rescale_block_flat,
    input  wire [4095:0]  partial_o_block_flat,
    output reg            oacc_row_rd_en,
    output reg  [1:0]     oacc_row_rd_addr,
    input  wire           oacc_row_rd_valid,
    input  wire [1023:0]  oacc_row_rd_data,
    output reg            oacc_row_wr_en,
    output reg  [1:0]     oacc_row_wr_addr,
    output reg  [1023:0]  oacc_row_wr_data,
    output reg            resp_valid,
    input  wire           resp_ready,
    output reg            done_pulse
    ...
    reg [1:0] row_idx_r;
    reg [1:0] row_idx_n;
    reg [127:0] rescale_block_flat_r;
    reg [4095:0] partial_o_block_flat_r;
    reg [1023:0] row_new_data_w;
    reg          oacc_row_rd_en_n;
    reg [1:0]    oacc_row_rd_addr_n;
    reg          oacc_row_wr_en_n;
    reg [1:0]    oacc_row_wr_addr_n;
    reg [1023:0] oacc_row_wr_data_n;
    reg          resp_valid_n;
    reg          done_pulse_n;
    reg signed [31:0] scale_raw_s;
    ...
    always @(*) begin
        row_new_data_w = 1024'd0;
        scale_raw_s = rescale_block_flat_r[(row_idx_r * 32) +: 32];
        for (elem_i = 0; elem_i < 64; elem_i = elem_i + 1) begin
            partial_q88_s = partial_o_block_flat_r[(((row_idx_r * 32) + (elem_i >> 1)) * 32) +
                                                   ((elem_i & 1) * 16) +: 16];
            row_new_data_w[(elem_i * 16) +: 16] = update_oacc_elem(
                oacc_row_rd_data[(elem_i * 16) +: 16],
                scale_raw_s,
    ...
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            state_r <= ST_IDLE;
            row_idx_r <= 2'd0;
            rescale_block_flat_r <= 128'd0;
            partial_o_block_flat_r <= 4096'd0;
            oacc_row_rd_en <= 1'b0;
            oacc_row_rd_addr <= 2'd0;
            oacc_row_wr_en <= 1'b0;
            oacc_row_wr_addr <= 2'd0;
`ifndef SYNTHESIS
            oacc_row_wr_data <= 1024'd0;
`endif
            resp_valid <= 1'b0;
...
`endif
            ...
            row_idx_r <= row_idx_n;
            if (req_valid && req_ready) begin
                rescale_block_flat_r <= rescale_block_flat;
                partial_o_block_flat_r <= partial_o_block_flat;
            end
            oacc_row_rd_en <= oacc_row_rd_en_n;
            oacc_row_rd_addr <= oacc_row_rd_addr_n;
