...
module FA_ROW_STATE_Q4_BLOCK_REAL (
    input  wire          clk,
    input  wire          rstn,
    input  wire          clear,
    input  wire          init_valid,
    ...
    output reg           resp_valid,
    input  wire          resp_ready,
    output reg  [1023:0] p_block_flat,
    output reg  [127:0]  rescale_block_flat,
    output reg           done_pulse,
    input  wire          restore_valid,
    input  wire [127:0]  restore_m_state_flat,
    input  wire [127:0]  restore_l_state_flat,
    input  wire [3:0]    restore_row_seen,
    output wire [127:0]  debug_m_state_flat,
    output wire [127:0]  debug_l_state_flat,
    output wire [3:0]    debug_row_seen
);
    ...
    reg signed [31:0] m_state_r [0:3];
    reg signed [31:0] l_state_r [0:3];
    reg               row_seen_r [0:3];
    ...
    wire [31:0] recip_out_value_w;
    wire        recip_done_pulse_w;
    wire        rowstate_unused_zero_w =
        (recip_done_pulse_w & 1'b0) | (alpha_r[0] & 1'b0) | (sum_beta_r[0] & 1'b0);
...
`include "fa_exp_lut_q16_16.vh"
    ...
    generate
        genvar gi;
        for (gi = 0; gi < 4; gi = gi + 1) begin : gen_debug_flat
            assign debug_m_state_flat[(gi * 32) +: 32] = m_state_r[gi];
            assign debug_l_state_flat[(gi * 32) +: 32] = l_state_r[gi];
            assign debug_row_seen[gi] = row_seen_r[gi];
        end
    ...
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            masked_score_block_flat_r <= 2048'd0;
`ifndef SYNTHESIS
            p_block_flat <= 1024'd0;
            rescale_block_flat <= 128'd0;
`endif
            init_done_pulse <= 1'b0;
            resp_valid <= 1'b0;
            done_pulse <= 1'b0;
                ...
                ST_ROW_COMMIT: begin
                    if (!row_has_valid_r) begin
                        for (col_i = 0; col_i < 16; col_i = col_i + 1) begin
                            ...
                            lane_i = col_i & 1;
                            p_block_flat[(word_i * 32) + (lane_i * 16) +: 16] <= 16'd0;
                        end
                        if (row_has_history_r) begin
                            rescale_block_flat[(row_idx_r * 32) +: 32] <= Q16_ONE;
                        end else begin
                            rescale_block_flat[(row_idx_r * 32) +: 32] <= Q16_ZERO;
                        ...
                        rescale_q16_s = q16_mul_rn_sat(alpha_l_old_r, recip_l_new_r);
                        rescale_block_flat[(row_idx_r * 32) +: 32] <= rescale_q16_s;
                        for (col_i = 0; col_i < 16; col_i = col_i + 1) begin
                            p_q16_s = q16_mul_rn_sat(beta_r[col_i], recip_l_new_r);
                            p_q88_s = q16_to_q88_rn_sat(p_q16_s);
                            word_i = ((row_idx_r * 16) + col_i) >> 1;
                            lane_i = col_i & 1;
                            p_block_flat[(word_i * 32) + (lane_i * 16) +: 16] <= p_q88_s;
                        end
